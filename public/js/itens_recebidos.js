const ReceivedItemsModule = {
    ...
    updateFilters() {
        this.state.startDate = document.getElementById('filter-start')?.value || this.state.startDate;
        this.state.endDate = document.getElementById('filter-end')?.value || this.state.endDate;
        this.fetchData();
    },
    ...
    renderList() {
        ...
        container.innerHTML = this.state.lists.map(list => {
            ...
            const categoria = String(list.Categoria || '');

            let colorClass = 'border-l-4 border-gray-400';
            if (categoria.includes('Almoço')) colorClass = 'border-l-4 border-orange-500';
            else if (categoria.includes('Jantar')) colorClass = 'border-l-4 border-indigo-500';
            else if (categoria.includes('Sopa')) colorClass = 'border-l-4 border-green-500';
                ...
                <div class="bg-white rounded-lg shadow-sm border ${colorClass} overflow-hidden">
                    <div class="p-4 flex justify-between items-center cursor-pointer hover:bg-gray-50 transition" onclick="ReceivedItemsModule.toggleDetails('${list.ID}')">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="font-bold text-lg text-gray-800">${this.formatCategory(categoria)}</h4>
                            ...
                            <tbody>
                                ${itens.map(item => `
                                    <tr class="border-b last:border-0 border-gray-200">
                                        ...
                                        <td class="py-2 text-center font-bold text-gray-800">${item.qtd || 0}</td>
    ...
    formatCategory(cat) {
        return String(cat || '-').replace('SopaCha', 'Sopa & Chá - ');
    }
};

window.ReceivedItemsModule = ReceivedItemsModule;
